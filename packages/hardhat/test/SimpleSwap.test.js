...
describe("SimpleSwap", function () {
  it("Should provide a caller with more DAI than they started with after a swap", async function () {
    try {
      ...
      const deposit = await weth9.deposit({ value: ethers.utils.parseEther("100"), gasLimit: 500000 });
      await deposit.wait();
      ...
      await weth9.approve(simpleSwap.address, ethers.utils.parseEther("100"));
      ...
      try {
        const swap = await simpleSwap.swapWETHForDai(amountIn, { gasLimit: 500000 });
        await swap.wait();
